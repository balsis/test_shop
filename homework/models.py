...
class Product:
    ...
    def buy(self, quantity):
        ...
        """
        if quantity < 0 or not self.check_quantity(quantity):
            raise ValueError("Продуктов не хватает или некорректное значение")
        self.quantity -= quantity
...
class Cart:
    ...
    def add_product(self, product: Product, buy_count: int = 1):
        ...
        """
        if not isinstance(buy_count, int) or buy_count <= 0:
    ...
    def remove_product(self, product: Product, remove_count: Optional[int] = None):
        """
        ...
        if product not in self.products:
            raise ValueError(f"Продукт {product.name} не найден в корзине.")
